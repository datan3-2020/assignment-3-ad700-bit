...
setwd("C:/Users/Adam/OneDrive/Documents/assignment-3-ad700-bit")
getwd()
```
...
files <- dir(
             ...
             "C:/Users/Adam/OneDrive/Documents/University Academic 2019-20/Classes Term 2/Data Analysis 3/Data/UKDA-6614-tab/tab",
...
Long <- all7 %>% melt(id = "pidp",value.name = "value", na.rm = TRUE) %>%
  separate(variable,
           into = c("wave", "variable"),
           sep = "_",
           fill = "right") %>%
...
Long <- Long %>%
filter(memorig == 1) %>%
mutate(sex = recode(sex, '1' = "male", '2' = "female", .default = NA_character_)) %>%
mutate(vote6 = replace(vote6, vote6 <= "0", NA)) %>%
mutate(vote6 = replace(vote6, vote6 >= "5", NA))

Long <- na.omit(Long)

x <- ifelse(Long$sex == "female", 1, ifelse(Long$sex == "male", 2, NA))

Long %>%tabulate(bin = x)
  
Long %>%
        ...
## Do not understand why it is not working. Error = the condition has length > 1 and only the first element will be usedError in tabulate(., bin = x) : 'list' object cannot be coerced to type 'integer'. Have searched the internet however can not find a solution.
...
```{r}
meanVote6 <- meanVote6 <- Long %>%
  group_by(wave, sex) %>%
  summarise(average = mean(vote6))
        ...
...
```{r}
meanVote6 <- meanVote6 %>% pivot_wider(wave:average, names_from = "wave",
                                       values_from = "average")
meanVote6

#It would appear females have a consistantly higher political interest average than men do. In all waves females scored higher than men did. Wave D is most noticable for having the highest overall average in both men and women than any other age. This will likely be as a result of a political event or interaction at the time which would have caused an increase in political intrest. My guess is that this would have been the EU referendum.
```
